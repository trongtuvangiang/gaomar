...
import jp.gaomar.nagoyacitybus.DestinationAdapter;
import jp.gaomar.nagoyacitybus.R;
import jp.gaomar.nagoyacitybus.TimetableActivity;
		...
		AsyncTask<Void, Void, List<BusDestination>> task = new AsyncTask<Void, Void, List<BusDestination>>() {
			...
			private ArrayList<BusDestination> parseDocument(Document document) {
				...
				for (Element anchor : anchorTag) {
					String[] name = anchor.getElementsByTag("img").first().attr("alt").split("、");
					String route = name[0];					
					String distination = name[1].substring(0, name[1].indexOf("の時刻表を表示"));
