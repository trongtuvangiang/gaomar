...
import jp.gaomar.fragment.TimetableFragment;
...
public class TimetableActivity extends FragmentActivity{
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (extras!=null) {
			...
			String note = getNote(baseData.getHtml());
			if ("".equals(note)) {
	...
	private String getNote(String html) {
		String note = "";
		Document document = Jsoup.parse(html);
		
		Element content = document.getElementById("weekday");
		note = content.getElementsByClass("sign").get(0).text();
